  ...
  try {
    ...
    try {
      ...
      try {
        ...
        await pool.query(`UPDATE users SET role='ADMIN', updated_at=now() WHERE email='admin@example.local' AND role<>'ADMIN'`)
        await pool.query(`UPDATE users SET password_hash=$1, updated_at=now() WHERE email='admin@example.local' AND password_hash IS NULL`, [adminHash])
      } catch (e) {
