  ...
  try {
    ...
    try {
      ...
      try {
        ...
        await pool.query(
          `INSERT INTO users (username, email, password_hash, role) VALUES ('arrofi','admin@example.local',$1,'ADMIN') ON CONFLICT (email) DO UPDATE SET role='ADMIN' WHERE users.email='admin@example.local' AND users.role<>'ADMIN'`,
          [adminHash],
        )
      } catch (e) {
